...
// Maximum percentage from this program is 90%. (10% of grade is documentation.)
  ...
  if (s.sum( ) || t.sum( ))
{
  ...
  if (!(t == v))
       {
      cout << "5" << endl;
      cout << t.sum() << "/ /" << t.length() << "/ /" << t.minimum() << "/ /" << t.maximum() << "-------" << v.sum() << "/ /" << v.length() << "/ /" << v.minimum() << "/ /" << v.maximum() << "______" << s.length() << endl;
      return 0;
  }
  v = t + u;
  if (v.length( ) != 5)
...
int main( )
{
  ...
  cerr << "TEST 1:" << endl;
  cerr << "Testing next, reset, length, sum, and mean (62%).\n";
  result = test1( );
  value += result;
  ...
  cerr << "\nTEST 2:" << endl;
  cerr << "Testing minimum and maximum member functions (7%).\n";
  result = test2( );
  value += result;
  ...
  cerr << "grade will be " << value << "% out of 90%.\n";
